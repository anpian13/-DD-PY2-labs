...
class Library:
    ...
    def get_index_by_book_id(self, target_book_id) -> int:
        if not isinstance(target_book_id, int):
            raise TypeError("Id искомой книги должен быть типа int")
        for index, item in enumerate(self.books):
            if self.books[index].id_ == target_book_id:
                return index
            else:
